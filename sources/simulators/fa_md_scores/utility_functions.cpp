...
void write_score_fa(const vector<uint32_t> & score, mat_t * score_mat) {
    size_t dims[2] = {1U, score.size()};

    matvar_t * score_fa_var = Mat_VarCreate(
        "score_fa",
        MAT_C_UINT32, MAT_T_UINT32,
        2, dims,
        (void *) score.data(),
        0);
    if (!bool(score_fa_var)) {
...
void write_score_md(const vector<uint32_t> & score, mat_t * score_mat) {
    size_t dims[2] = {1U, score.size()};
...
void write_full_score(const vector<vector<uint32_t>> & scores, mat_t * score_mat) {
    size_t dims[2] = {scores[0].size(), scores.size()};

    uint32_t * lin_scores = new uint32_t[dims[0] * dims[1]];
    for (int i = 0; i < int(scores.size()); i++) {
        memcpy(lin_scores + i * scores[0].size(), scores[i].data(), scores[0].size() * sizeof(uint32_t));
    }
...
void write_full_score(const vector<vector<uint32_t>> & scores, FILE * score_file) {
    for (const auto & score : scores) {
        for (const auto & value : score) {
            fprintf(score_file, "%-16u ", value);
        }
        fprintf(score_file, "\n");
    }
}
