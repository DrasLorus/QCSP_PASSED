...
TEST_CASE("CNormGeneric (L2) works for high snr inputs (q: 64)", "[normgeneric][high][l2]") {
    ...
    CNormGeneric * proc = new CNormGeneric(q);
...
TEST_CASE("CNormGeneric (L2) works for low snr inputs (q: 64)", "[normgeneric][low][l2][q64]") {
    ...
    // Initialization Startup
    for (int64_t i = 0; i < int64_t(q - 1); i++) {
        REQUIRE_THAT(results[i], Catch::Matchers::WithinRel(norms_out[i] * norms_out[i] + 1, float(1e-5f)));
    }

    for (int64_t i = q - 1; i < int64_t(results.size()); i++) {
        REQUIRE_THAT(std::sqrt(results[i]), Catch::Matchers::WithinRel(norms_out[i], float(5e-4f)));
    }
