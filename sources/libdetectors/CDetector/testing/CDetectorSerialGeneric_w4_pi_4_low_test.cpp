...
TEST_CASE("CDetectorSerialGeneric (L2, float) works for low snr inputs (q: 64, N: 60, w: 4, d: 4)", "[detectorgeneric][low][l2][w4][q64]") {
    ...
    mat_t * data_file = Mat_Open("../data/test_data_q64_N60_w4_step4_span0.5.mat", MAT_ACC_RDONLY);
    if (not bool(data_file)) {
        throw "../data/test_data_q64_N60_w4_step4_span0.5.mat can't be opened.";
    }
...
TEST_CASE("CDetectorSerialGeneric (Raw, float) works for low snr inputs (q: 64, N: 60, w: 4, d: 4)", "[detectorgeneric][low][raw][w4][q64]") {
