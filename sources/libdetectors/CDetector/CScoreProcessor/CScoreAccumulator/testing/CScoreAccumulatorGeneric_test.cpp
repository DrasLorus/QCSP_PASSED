...
TEST_CASE("CScoreAccumulatorGeneric works for high snr inputs (q: 64, N: 60)", "[scoreaccugeneric][high]") {
    ...
    CScoreAccumulatorGeneric proc(q, N);
...
TEST_CASE("CScoreAccumulatorGeneric works for low snr inputs (q: 64, N: 60)", "[scoreaccugeneric][low][q64][N60]") {
    ...
    CScoreAccumulatorGeneric * proc = new CScoreAccumulatorGeneric(q, N);
    ...
    for (int64_t i = 0; i < q * N; i++) {
        INFO("input no " << i);
        REQUIRE_THAT(results[i], Catch::Matchers::WithinAbs(score_out[i], float(1e-4f)));
    }

    for (int64_t i = q * N; i < int64_t(results.size()); i++) {
        INFO("input no " << i);
        REQUIRE_THAT(results[i], Catch::Matchers::WithinRel(score_out[i], float(1e-4f)));
    }
