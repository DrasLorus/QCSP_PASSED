...
TEST_CASE("CCorrAbsMaxGeneric works for high snr inputs (q: 64)", "[corrabsmaxgeneric][high][q64]") {
    ...
    CCorrAbsMaxGeneric * proc = new CCorrAbsMaxGeneric(pn);
...
TEST_CASE("CCorrAbsMaxGeneric works for low snr inputs (q: 64)", "[corrabsmax][low][q64]") {
    ...
    for (int64_t i = 0; i < int64_t(results.size()); i++) {
        REQUIRE_THAT(results[i], Catch::Matchers::WithinRel(cabs_out[i], float(1e-4f)));
    }
